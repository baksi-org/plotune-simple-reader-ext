...
if __name__ == "__main__":
    # quick demo: simulate three async sockets feeding different rates
    import random
    import math

    async def simulate_socket(name: str, writer: AsyncPltWriter, rate_hz: float, duration_s: float):
        sid = writer.add_signal_meta(name, unit="unit", description=f"{name} desc", source="SIM")
        period = 1.0 / rate_hz
        t0 = time.time()
        while time.time() - t0 < duration_s:
            ts = time.time()
            val = 10.0 + math.sin(ts * 2*math.pi*0.7) + random.random()*0.01
            await writer.record_point(sid, ts, val)
            await asyncio.sleep(period)

    async def main():
        writer = AsyncPltWriter(
            "demo_output_expand.pltx",
            compression="zstd",
            chunk_records=512,
            flush_interval_sec=0.25,
            fsync_every_n_chunks=4,
        )
        await writer.start()

        # three signals, different rates
        tasks = [
            asyncio.create_task(simulate_socket("Voltage", writer, rate_hz=800, duration_s=5*60)),
            asyncio.create_task(simulate_socket("Current", writer, rate_hz=120, duration_s=5*60)),
            asyncio.create_task(simulate_socket("Power",   writer, rate_hz=250, duration_s=5*60)),
            asyncio.create_task(simulate_socket("RPM", writer, rate_hz=200, duration_s=5*60)),
            asyncio.create_task(simulate_socket("Frequency", writer, rate_hz=500, duration_s=5*60)),
            asyncio.create_task(simulate_socket("Torque",   writer, rate_hz=250, duration_s=5*60)),
        ]
        await asyncio.gather(*tasks)

        # user hits SAVE
        await writer.stop_and_save()
        print("Saved:", writer.final_path)
        print("Signals:", writer.list_registered_signals())

        # read back
        r = PltReader(writer.final_path)
        sig_map = {name: sid for sid, name in r.list_signals()}
        v_id = sig_map["Voltage"]
        # read a small window (last 0.3s)
        t_end = time.time()
        t_start = t_end - 0.3
        total = 0
        for ts_chunk, val_chunk in r.iter_time_range(v_id, t_start, t_end):
            total += len(ts_chunk)
        print("Voltage points in last 0.3s:", total)
        r.close()
