...
class SimpleReaderAgent:
    ...
    async def read_file(self, request: FileReadRequest):
        ...
        if assigned_signals is None:
            raise HTTPException(
                status_code=400, detail=f"Unsupported file format for {request.path}"
            )
        ...
        meta = FileMetaData(
            ...
            tags=tags,
            created_at=(
                datetime.fromtimestamp(header["created"])
    ...
    async def stream(
        ...
        try:
            ...
            if orig_signal_name not in header.get("signal_names", []):
                raise ValueError(
                    f"Signal '{signal_name}' (original: '{orig_signal_name}') not found in file."
                )
