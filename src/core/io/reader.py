from core.io.pltx_convertion import PltReader
import pandas as pd
import json
...
class Reader:
    ...
    def get_header(self) -> dict:
        if self.engine == "pltx":
            return {
                ...
                "signal_names": [
                    meta["name"] for sid, meta in self.reader.signals.items()
                ],
            }
    ...
    def _standard_header(self, signal_names: list) -> dict:
        ...
        return {
            ...
            "created": None,
            "signals": {
                str(i + 1): {"name": name} for i, name in enumerate(signal_names)
            },
            "signal_names": signal_names,
        }

    def iter_chunks(
        self, signal_name: str, chunk_size: int = 1000
        ...
        if self.engine == "pltx":
            ...
            for batch in self.parquet_file.iter_batches(
                batch_size=chunk_size, columns=["Time", signal_name]
            ):
